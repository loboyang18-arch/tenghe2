# -*- coding: utf-8 -*-
"""
exog_forecaster_v0_4_1_clean.py

Clean, leakage-safe evaluation for forecasting future exogenous *actual* series (15min × H steps).

Core rules
----------
1) Target (actual) is NEVER imputed. Any sample whose future labels (t+1..t+H) contains NaN is dropped.
2) For each target, truncate dataframe to the target's last valid timestamp (avoid tail-NaN zones).
3) Optional '预测值' columns are FEATURES; they may be imputed via --feat_nan_strategy.
4) Split is chronological by date of (decision_time + H*step).

Methods
-------
- seasonal_median: median by (weekday, minute_of_day) computed on TRAIN actuals only
- persist: last observed at decision time repeated H steps
- use_pred: if pred col exists, use pred[t+1..t+H] directly (NaN filled by seasonal fallback)
- ridge_pure: Ridge multi-output using history window (target history + optional pred history)
- ridge_residual: if pred col exists, learn residual (actual - pred) using [history + future pred seq] features
- hgbt_pure / hgbt_residual (optional): HistGradientBoostingRegressor via MultiOutputRegressor

Outputs under outputs/<tag>/:
- suite_summary_clean.csv
- per target / per method:
  - metrics.json
  - test_horizon_mae.png
  - test_last7d_h24.png
  - test_predictions_long.csv

Example:
python -u exog_forecaster_v0_4_1_clean.py --excel "山东-全年-带时间点.xlsx" --time_col "datetime" \
  --target_cols "系统负荷实际值,光伏实际值,联络线实际值,上旋备用实际值,下旋备用实际值" \
  --lookback 192 --H 24 --step_minutes 15 --val_days 5 --test_days 6 \
  --feat_nan_strategy ffill --ridge_alpha 10 \
...
def main():
    ...
    if all_rows:
        ...
        print("[Done] No results produced (no samples / missing targets).")
